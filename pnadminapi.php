...
function iw_agendas_adminapi_editAgenda($args)
{
    ...
    if (!pnModFunc('iw_main', 'user', 'checkSecurityValue',
                   array('sv' => $sv)) || !SecurityUtil::checkPermission('iw_agendas::', "::", ACCESS_READ)) {
        // Security check
...
function iw_agendas_adminapi_create($args)
{
    ...
    $sv = FormUtil::getPassedValue('sv', isset($args['sv']) ? $args['sv'] : null, 'POST');
    if (!pnModFunc('iw_main', 'user', 'checkSecurityValue',
                    array('sv' => $sv)) || !SecurityUtil::checkPermission('iw_agendas::', "::", ACCESS_READ) || $gCalendarId == '') {
        // Security check
        if (!SecurityUtil::checkPermission('iw_agendas::', "::", ACCESS_ADMIN)) {
            return LogUtil::registerError(__('Sorry! No authorization to access this module.', $dom), 403);
        }
...
function iw_agendas_adminapi_delete($args)
{
    ...
    if(!pnModFunc('iw_main', 'user', 'checkSecurityValue',
                   array('sv' => $sv)) || !SecurityUtil::checkPermission('iw_agendas::', "::", ACCESS_READ) || $item['resp'] != '$$' . pnUserGetVar('uid') . '$'){
        // Security check
        if (!SecurityUtil::checkPermission('iw_agendas::', "::", ACCESS_ADMIN)) {
            return LogUtil::registerError(__('Sorry! No authorization to access this module.', $dom), 403);
        }
