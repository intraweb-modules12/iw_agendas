...
function iw_agendas_admin_main()
{
    ...
    $agendas = pnModAPIFunc('iw_agendas','user','getAllAgendas',
                             array('onlyShared' => 1));
...
function iw_agendas_admin_create($args)
{
    ...
    //Update the agenda
    $lid = pnModAPIFunc('iw_agendas', 'admin', 'create',
...
function iw_agendas_admin_update($args)
{
    ...
    $items = array('nom_agenda' => $nom_agenda,
                   ...
                   'protegida' => $protegida,
                   'color' => $color);
    $lid = pnModAPIFunc('iw_agendas', 'admin', 'editAgenda',
                         array('daid' => $daid,
                               'items' => $items));
    if ($lid != false) {
...
function iw_agendas_admin_deleteManager($args)
{
    ...
    if(!$confirm){
        $sv = pnModFunc('iw_main', 'user', 'genSecurityValue');
        $userName = pnModFunc('iw_main', 'user', 'getUserInfo',
                               array('sv' => $sv,
                                     'uid' => $id,
                                     'info' => 'ncc'));
        // Create output object
...
function iw_agendas_admin_nouallista($args){
    ...
    if (empty($any1)) $any1 = date('Y');
    if (empty($any2)) $any2 = date('Y');
...
function iw_agendas_admin_modifica_llistes($args)
{
    ...
    if ($index == 3) {
        ...
        array_shift($periodes);
        //Check the colour
        if (strlen($color) != 7) $colorerror = true;
