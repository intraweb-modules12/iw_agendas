...
function iw_agendas_userapi_crear($args)
{
    ...
    pnModCallHooks('item', 'create', $items['aid'],
                    array('module' => 'iw_agendas'));
    $pntable = pnDBGetTables();
    $c = $pntable['iw_agendas_column'];
...
function iw_agendas_userapi_subsalta($args)
{
    ...
    if (count($item) < 1) {
        $items = array('daid' => $daid,
                       'uid' => $uid,
                       'donadabaixa' => '-1');
        if (!DBUtil::insertObject($items, 'iw_agendas_subs', 'said')) $error = true;
    } else {
        $items = array('donadabaixa' => '-1');
        if (!DBUTil::updateObject ($items, 'iw_agendas_subs', $where)) $error = true;
    }
    if ($error) {
