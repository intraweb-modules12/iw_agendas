...
function iw_agendas_ajax_protectNote($args)
{
    ...
    $note = pnModAPIFunc('iw_agendas', 'user', 'get',
                          array('aid' => $aid));
    if ($note == false) AjaxUtil::error(__('Event not found', $dom));
    if($note['daid'] != 0){
    ...
    if (strpos($agenda['gAccessLevel'], '$owne|' . pnUserGetVar('uid') . '$') === false &&
        ...
        if ($daid != 0) {
            ...
            if ($te_acces < 3 || ($te_access == 3 && $anotacio['usuari'] != pnUserGetVar('uid')))  AjaxUtil::error(__('You are not allowed to administrate the agendas', $dom));
        } else {
            //Comprovem si l'usuari estÃ  protegint realment la seva a notaciÃ³
            if ($note['usuari'] != pnUserGetVar('uid')) AjaxUtil::error(__('You are not allowed to administrate the agendas', $dom));
        }
    }
...
function iw_agendas_ajax_modifyAgenda($args)
{
    ...
    $item = pnModAPIFunc('iw_agendas', 'user', 'getAgenda',
                                          array('daid' => $daid));
    if ($item == false) AjaxUtil::error(DataUtil::formatForDisplayHTML(__('The agenda was not found', $dom)));
